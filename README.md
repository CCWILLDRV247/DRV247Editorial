...
On **For You**, tap **Set test** to pick a catalog make/model, interests, and location. That selection is stored in the URL and in `localStorage` (`drv247-for-you-test`). Ranking only uses entities/interests/locations already on the story — it will not invent a Porsche match for a story with no Porsche metadata. Extraction is rule-based from title, teaser, and the stored first-paragraph extract: 964/993/996/GT3 count as 911, 355 GTB as Ferrari F355. This is a placeholder until the real DRV247 personalization string exists. **Clear test** removes it.
