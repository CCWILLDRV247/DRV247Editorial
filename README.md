...
- **Vehicle Intelligence** — `/intelligence` (Build + Maintain; Design 911 + Eurospares) and `/intelligence/print` (physical-magazine newsstand). Not on the magazine nav. Live catalogues refresh on `15 6 * * 1` → `/api/cron/intelligence-ingest`. Print is a curated publication store, not RSS ingest.
